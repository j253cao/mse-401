...
def recommend(request: QueryRequest):
    ...
    t1 = time.time()
    # Data/model load is inside get_recommendations
    results = get_recommendations(request.queries, data_file='waterloo-open-api-data.json', method='cosine')
    t2 = time.time()
...
def resume_recommend(file: UploadFile = File(...)):
    t0 = time.time()
    ...
    try:
        ...
        recommendations = get_recommendations([query], data_file='waterloo-open-api-data.json', method='cosine')
        formatted = [
            {
                "rank": r["rank"],
                "course_code": r["course_code"],
