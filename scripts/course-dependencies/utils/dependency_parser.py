...
def parse_prerequisites(prerequisites_html):
    ...
    if re.search(all_pattern, prerequisites_html, re.IGNORECASE):
        ...
        if all_match:
            ...
            must_all_nested_matches = re.findall(must_all_nested_pattern, all_content, re.DOTALL | re.IGNORECASE)
            ...
            enrolled_nested_matches = re.findall(enrolled_nested_pattern, all_content, re.DOTALL | re.IGNORECASE)
            ...
            # Remove the nested blocks from all_content so they aren't double-counted
            all_content_cleaned = re.sub(must_all_nested_pattern, '', all_content, flags=re.DOTALL | re.IGNORECASE)
            ...
            complete_one_matches = re.findall(complete_one_pattern, all_content_cleaned, re.DOTALL)
...
def parse_corequisites(html_content):
    ...
    if unique_codes:
        return {'type': 'one', 'rules': [{'type': 'course_requirement', 'code': code} for code in unique_codes]}
...
def parse_antirequisites(html_content, departments=None, courses=None):
    ...
    if valid_codes:
        all_rules.extend([{'type': 'course_requirement', 'code': code} for code in valid_codes])
