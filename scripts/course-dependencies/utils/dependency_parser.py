    ...
    if re.search(all_pattern, prerequisites_html, re.IGNORECASE):
        ...
        if all_match:
            ...
            for i, course_code in enumerate(remaining_must_complete_matches):
                ...
                print(f"DEBUG: Found individual course: {clean_code}")
                all_items.append({
            ...
            for item in all_items:
                if item['type'] == 'one' or item['type'] == 'all':
                    for rule in item.get('rules', []):
                        if rule.get('code'):
                            found_codes.add(rule['code'])
                elif item.get('code'):
            ...
            print(f"DEBUG: Found codes to exclude: {found_codes}")
            ...
            remaining_rules = extract_simple_rules(content_for_simple_rules, exclude_codes=found_codes)
            if remaining_rules:
                print(f"DEBUG: Found {len(remaining_rules)} remaining simple rules (excluding already found)")
                for rule in remaining_rules:
                    if rule.get('code'):
                        print(f"DEBUG: Found course code: {rule['code']}")
                all_items.append({'type': 'one', 'rules': remaining_rules})
